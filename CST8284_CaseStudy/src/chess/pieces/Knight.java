...
public final class Knight extends Piece {
	...
	public void findMoves(Square current) {
		...
		if (x-2 >= 0) {
			if (y-1 >= 0) tempMoves.add(Board.boardArray[x-2][y-1]);
			if (y+1 <= 7) tempMoves.add(Board.boardArray[x-2][y+1]);
		}
		
		if (x-1 >= 0) {
			if (y-2 >= 0) tempMoves.add(Board.boardArray[x-1][y-2]);
			if (y+2 <= 7) tempMoves.add(Board.boardArray[x-1][y+2]);
		}
